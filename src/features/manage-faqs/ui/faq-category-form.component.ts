...
export interface FaqCategoryFormOutput {
  companyId: string;
  formValue: CategoryFormValue;
}

@Component({
  selector: 'bk-faq-category-form',
  standalone: true,
...
export class FaqCategoryFormComponent {
  ...
  readonly saved = output<FaqCategoryFormOutput>();
  ...
  constructor() {
    effect(() => {
      ...
      if (data) {
        ...
        this.form.reset({ CompanyId: defaultId, BIsService: false });
      }
    });
  }
