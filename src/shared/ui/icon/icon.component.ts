...
@Component({
  ...
  template: `<span class="bk-icon" [class]="hostClasses()" [innerHTML]="svgContent()"></span>`,
  styles: `
    .bk-icon {
      display: inline-flex;
      align-items: center;
...
export class BkIconComponent {
  ...
  readonly hostClasses = computed(() => `bk-icon bk-icon-${this.size()}`);

  readonly svgContent = computed((): SafeHtml => {
    const raw = ICON_MAP[this.name()] ??
