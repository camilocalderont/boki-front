...
### 7. Auth interceptor desacoplado

El auth interceptor en `shared/api/` usa `InjectionToken<() => string | null>` para obtener el token, en vez de importar directamente desde `@features/auth`. Esto mantiene `shared/` independiente de capas superiores.

```typescript
// shared/api/auth.tokens.ts
export const AUTH_TOKEN_GETTER = new InjectionToken<() => string | null>('AUTH_TOKEN_GETTER');

// En app.config.ts, se provee la implementación concreta:
// { provide: AUTH_TOKEN_GETTER, useFactory: () => inject(AuthStore).token }
```

### 8. Theming via CSS custom properties
